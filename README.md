...
| Day 2 workbook | The hands-on copy you fill in during the session | [![Open in Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/shreyasgm/ecu-complexity-labs/blob/main/day2/day2_workbook.ipynb) |
| Day 2 reference, student edition | Shows what each step produces, with the answer code blanked | [![Open in Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/shreyasgm/ecu-complexity-labs/blob/main/day2/day2_reference_student.ipynb) |
...
Each day's folder carries an R notebook (`day1/day1_reference_r.ipynb`, `day2/day2_reference_r.ipynb`) repeating the day's analysis with the economiccomplexity package, so R users can see that the package reproduces the hand-rolled numbers. They are read-along companions: view them here on GitHub or through the `.html` copies rather than running them on Colab, since they expect the packages and data paths of a local setup.

## The data

The notebooks download their data automatically from this repository's [data release](https://github.com/shreyasgm/ecu-complexity-labs/releases/tag/data-v1). To get the files yourself, download the zips from that release and unzip them into a folder named `morocco_data_packet` next to the notebook (each archive carries its folder name inside). Days 1 and 2 use `exports.zip` and `reference.zip`; the innovation dimensions (`patents.zip`, `publications.zip`, `trademarks.zip`, `cross_domain.zip`) and the subnational employment packet (`subnational_data_packet.zip`, United States and Mexico) come up later in the week. The product-space layout files Day 2 draws on were added to `reference.zip` on the morning of 7 July, so if you fetched the packet before then, delete your `morocco_data_packet` folder and let the notebook re-download it. The Moroccan regional employment data (CNSS) is not public and reaches participants separately within the program.
